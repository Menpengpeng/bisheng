...
| —（无新增领域对象 / 表 / DAO） | F043-flow-assistant-sql-schema-cache | 工作流「助手」节点（内部类型 `agent`）既有数据库开关（`sql_agent`）增量：配置态选表（新增 2 个只读 `/workflow/db/*` API，不落库）、运行态 schema 预取注入 SQL Agent prompt 并按名收窄工具集、schema 的 Redis 缓存（TTL 24h + 手动刷新 + Redis 故障降级直连）；仅改造既有 `SqlAgentParams`（新增 `selected_tables`/`schema_cache_enabled` 两个有默认值字段）与 `SqlAgentAPIWrapper`（新增 2 个可选入参）；URI 拼装在 workflow 包内复用并新增 DM8 方言；不拥有任何领域对象、不新增表 / 迁移、不引入数据源管理；密码加密存储与 SQL 写操作强拦截明确排除 |
...
| F043-flow-assistant-sql-schema-cache | —（无前置 Feature） | 既有「助手节点 sql_agent 数据库开关」上的纯增量改造，不依赖其他 v2.6.0 Feature；新增只读 API `POST /workflow/db/tables`、`POST /workflow/db/schema/refresh`（无状态、不落库）；workflow 模块（105）新增错误码段位 10560–10563（连接失败 / 驱动缺失 / 超时 / 无有效表）；缓存键按 `tenant_id` 隔离、密码不进键与日志；新增 DM8（`dm+dmPython`，`?schema=`）方言；不新增领域对象 / 表 / 迁移 / 不变量；存量流程靠新字段默认值（空选表 / 缓存关）零行为变化 |
...
| 105 | workflow | F043 沿用现有 `common/errcode/flow.py`，新增错误码段位 **10560–10563**：`DbConnectionFailedError`(10560，连接失败) / `DbDriverMissingError`(10561，驱动缺失) / `DbInspectTimeoutError`(10562，取元数据超时) / `DbSchemaNoValidTableError`(10563，勾选表全部不存在)；不得与既有 10525-10550 / 10599 冲突 |
...
| 2026-09-17 | 登记 F043 工作流助手节点选表取 Schema + Schema 缓存：表 1 标注「无新增领域对象 / 表 / DAO」（既有 sql_agent 链路增量：配置态选表、运行态 schema 预取注入 + 工具收窄、Redis TTL 缓存 + 手动刷新 + 故障降级）、表 3 追加（无前置 Feature）；新增 2 个无状态只读 API `POST /workflow/db/tables`、`POST /workflow/db/schema/refresh`；新增 105 段错误码 10560–10563；新增 DM8 方言（`dm+dmPython` + `?schema=`）；缓存键含 tenant_id、密码不入键/日志；存量流程靠新字段默认值零行为变化；未新增不变量 / 表 / 迁移 | F043 |
